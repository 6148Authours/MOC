...
from core.custom_type_aliases import ReplayBufferSamples, RolloutBufferSamples
...
class BaseBuffer(ABC):
    ...
    def __init__(
        ...
        self.subgoal_shape = (11,)
        self.shaping_shape = (11,)
        self.init_shape = (11,)
        self.memory_shape = (64,)
        self.pos = 0
        self.full = False
        self.device = device
        self.n_envs = n_envs
...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.subgoals = np.zeros(
            (self.buffer_size, self.n_envs) + self.subgoal_shape, dtype=np.float32)
        self.shaping_rewards = np.zeros(
            (self.buffer_size, self.n_envs) + self.shaping_shape, dtype=np.float32)
        self.init_states = np.zeros(
            (self.buffer_size, self.n_envs) + self.init_shape, dtype=np.float32)
        self.memory = np.zeros(
            (self.buffer_size, self.n_envs) + self.memory_shape, dtype=np.float32)
        self.rewards = np.zeros(
            (self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(
        ...
        self.observations[self.pos] = np.array(obs).copy()
        self.actions[self.pos] = np.array(action).copy()
        self.subgoals[self.pos] = np.array(subgoal).copy()
        self.shaping_rewards[self.pos] = np.array(shape_reward).copy()
        self.init_states[self.pos] = np.array(init_state).copy()
        self.memory[self.pos] = np.array(memory).copy()
        self.rewards[self.pos] = np.array(reward).copy()
        self.dones[self.pos] = np.array(done).copy()
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> RolloutBufferSamples:
        data = (
            ...
            self.subgoals.reshape(-1, 11)[batch_inds],
            self.shaping_rewards.reshape(-1, 11)[batch_inds],
            self.init_states.reshape(-1, 11)[batch_inds],
            self.values[batch_inds].flatten(),
            self.log_probs[batch_inds].flatten(),
            self.advantages[batch_inds].flatten(),
            self.returns[batch_inds].flatten(),
            self.memory.reshape(-1, 64)[batch_inds],
        )
